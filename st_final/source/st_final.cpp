...
//            OSReport("Entry %d: 0x%x \n", i, fighter->m_moduleAccesser->getControllerModule().getButton().m_mask);
    ...
    enemyOverride.m_overrideKoopa = Archive_Override_Both;
    OSReport("sizeof(ArchiveOverride) = %d, m_overrideKoopa = %d \n", sizeof(ArchiveOverrideSetting), enemyOverride.m_overrideKoopa);
